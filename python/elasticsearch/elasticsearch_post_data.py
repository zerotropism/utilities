...
parser.add_argument(
    "--data_dir", help="directory where JSON are stored", type=str
)
args = parser.parse_args()

es_index = "discovery_dev_clean"  # equivalent to SQL database
es_type = (
    "_doc"  # equivalent to SQL table: mandatory to use _doc for text elements
)
es_id_num_init = 1
        ...
        if file.is_file():
            with open(args.data_dir + "/" + file.name) as json_file:
                ...
                if (res.status_code == 200) or (res.status_code == 201):
                    print(
                        "File {} correctly added in index {}".format(
                            file.name, es_index
                        )
                    )
                else:
                    print(
                        "ERROR during POSTING file {} error !".format(
                            file.name
                        )
                    )
                    exit(1)
