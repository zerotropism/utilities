...
def split(data_dir, tsv_in, split_ratio):
    ...
    data = pd.read_csv(data_dir+'/'+tsv_in, delimiter='\t', encoding='utf-8')
    ...
    train.to_csv(tsv_train, sep='\t', encoding='utf-8', index=False, quoting=csv.QUOTE_NONE)
    ...
    test.to_csv(tsv_test, sep='\t', encoding='utf-8', index=False, quoting=csv.QUOTE_NONE)
