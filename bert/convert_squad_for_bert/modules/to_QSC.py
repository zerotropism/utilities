    ...
    for i in range(len(json_dict["data"])):
        ...
        for j in range(len(json_dict["data"][i]["paragraphs"])):
            ...
            for k in range(len(json_dict["data"][i]["paragraphs"][j]["qas"])):
                ...
                current_question_en = json_dict["data"][i]["paragraphs"][j][
                    "qas"
                ][k]["question"]
                ...
                current_sentence_en = (
                    ...
                    + current_question_en
                    + " [SEP] "
                    + current_context_sub_en
                )
                current_sentence_fr = (
                    "[CLS] "
                    + current_question_fr
                    + " [SEP] "
                    + current_context_sub_fr
                )
                ...
                logging.debug(
                    " - main: ------------sentence_en[{}]: {}".format(
                        k, current_sentence_en
                    )
                )
                logging.debug(
                    " - main: ------------sentence_fr[{}]: {}".format(
                        k, current_sentence_fr
                    )
                )
                logging.debug(
        ...
        for j in range(len(json_dict["data"][i]["paragraphs"])):
            ...
            if contexts_random_index[0] != j:
                ...
                for k in range(
                    ...
                    current_question_en = json_dict["data"][i]["paragraphs"][
                        j
                    ]["qas"][k]["question"]
                    current_question_fr = en2fr.translate(current_question_en)
...
def build_questions_for_test_df(json_in):
    ...
    for i in range(len(json_dict["data"])):
        ...
        for j in range(len(json_dict["data"][i]["paragraphs"])):
            ...
            for k in range(len(json_dict["data"][i]["paragraphs"][j]["qas"])):
                current_sentence_en = json_dict["data"][i]["paragraphs"][j][
                    "qas"
                ][k]["question"]
